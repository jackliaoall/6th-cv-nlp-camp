...
class Raindrop_v2(nn.Module):
    ...
    def forward(self, src, static, times, lengths):
        ...
        """
        maxlen, batch_size = src.shape[0], src.shape[1]
        missing_mask = src[:, :, self.d_inp:int(2*self.d_inp)]#后36个是mask
        src = src[:, :, :int(src.shape[2]/2)]#时序特征
        n_sensor = self.d_inp

        src = torch.repeat_interleave(src, self.d_ob, dim=-1)#沿指定维度复制多少倍
        ...
        if step1 == False:
            ...
            adj = self.global_structure.cuda()
            adj[torch.eye(self.d_inp).byte()] = 1#对角线本来也是1啊
        ...
        if static is not None:
            output = torch.cat([output, emb], dim=1)#时序+时间+静态特征
        output = self.mlp_static(output)
